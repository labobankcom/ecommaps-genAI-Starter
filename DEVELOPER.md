...
## 4. مهارات وكلاء الذكاء الاصطناعي (AI Agent Skills) 🤖

هذا القالب مزود مسبقاً بهيكلية **Agent Skills Specification** لتوجيه مساعدي الذكاء الاصطناعي (مثل Cursor, Copilot, Claude) للعمل باحترافية تامة مع بيئة إيكومابس.

- **مسار التوجيه (Router):** يتم الدخول عبر `AGENTS.md` في المجلد الرئيسي، والذي يوجه الذكاء الاصطناعي لقراءة `.agents/skills/use-ecommaps/SKILL.md`.
- **المراجع (References):** يحتوي مسار المهارات على أدلة حصرية ودقيقة للذكاء الاصطناعي، تمنعه من اختراع (Hallucinate) أكواد غير موجودة، وتجبره على استخدام حزم `@ecommaps/client` و `shadcn/ui` والالتزام بالتنسيقات المنطقية (`ms-`, `me-`) لدعم الـ RTL.
- **كيف تستفيد منه كمطور؟:** لا تحتاج لأي إعداد. بمجرد فتح المشروع في محرر مدعوم بالذكاء الاصطناعي، اطلب منه: *"قم ببناء مكون عرض المنتجات بتصميم زجاجي"*، وسيقوم الوكيل آلياً بقراءة المهارات المدمجة واستخدام الكود الصحيح لـ Ecommaps.
